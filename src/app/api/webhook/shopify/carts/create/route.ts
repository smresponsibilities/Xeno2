...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log('Received cart creation webhook');

    // Verify HMAC
    if (!verifyShopifyHmac(req, Buffer.from(bodyString))) {
      console.error('Invalid HMAC for cart creation webhook');
    ...
    return NextResponse.json({ success: true, event: 'carts/create' });
  } catch (error) {
    console.error('Cart creation webhook processing error:', error);
